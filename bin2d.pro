PRO BIN2D, N, X1,X2,Y1,Y2, XARR,YARR, TAGS=TAGS, WEIGHT=WEIGHT, $ 
           XCONT,YCONT,VARIMG;, $
...
FOR I=0L, N1-1 DO BEGIN
    ...
    FOR J=0L, N2-1 DO BEGIN
        ...
        ;; CHECK FOR THE TAGES AND CREAT 2D HISTOGRAM
        IF (KEYWORD_SET(WEIGHT) && (N_ELEMENTS(WEIGHT) NE 0)) THEN BEGIN
            IF NINDEX GT 0 THEN $ 
               VARIMG(J,I)= TOTAL(WEIGHT[INDEX])        
        ENDIF ELSE BEGIN             
            IF NINDEX GT 0 THEN $ 
               VARIMG(J,I)= NINDEX        
        ENDELSE
